...
import com.example.ordering_lecture.item.dto.ItemQuantityUpdateDto;
...
public class ItemController {
    ...
    public ResponseEntity<ItemResDto> itemFindById(@PathVariable Long id){
        ItemResDto item = itemService.findById(id);
        System.out.println(item.getCategory());
        return new ResponseEntity<>(item,HttpStatus.CREATED);
    }

    @PostMapping("item/updateQuantity")
    public ResponseEntity<CommonResponse> itemUpdateQuantity(@RequestBody List<ItemQuantityUpdateDto> itemQuantityUpdateDtoList){
        itemService.updateQuantity(itemQuantityUpdateDtoList);
        return new ResponseEntity<>(
                new CommonResponse(
                        HttpStatus.CREATED, "item successfully created",null),
                HttpStatus.CREATED);
    }
